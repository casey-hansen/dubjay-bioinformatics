...
```{r}
# This code has been modified from plot-ranges.r 
# Copyright (C) 2014 Vince Buffalo <vsbuffaloAAAAA@gmail.com>
# This code has been modified using stack exchange: Question Title
# Define Ranges
x <- IRanges(start=c(4, 7, 2, 20), end=c(13, 7, 2, 23))
names(x) <- letters[1:4]
...
con <- file('~/Desktop/dubjay-bioinformatics/Chapters9-10/egfr_flank.fasta',open="r")
line <- readLines(con) 
...
```{r}
# Read in file, fill in blank spaces so dataframe has correct dimensions
df <- read.table('~/Desktop/dubjay-bioinformatics/Chapters9-10/egfr_flank.fasta',header=FALSE,sep="|",fill=TRUE)
```
...
# Read in FASTA file
fastaFile <- readDNAStringSet('~/Desktop/dubjay-bioinformatics/Chapters9-10/egfr_flank.fasta')
...
# Upload FASTQ file, identify indices of name, sequence, and score information
fastq <- read.table('~/Desktop/dubjay-bioinformatics/Chapters9-10/contam.fastq',header=FALSE,sep="|",fill=TRUE)
name <- seq(1, length(fastq$V1), by=4)
seq <- seq(2, length(fastq$V1), by=4)
score <- seq(4, length(fastq$V1), by=4)
...
# Transform score from character to ASCII number to PHRED number
# Save sequence scores in list in Score_val
for (each in fastq_data$Score_char){
  score_num = c()
  char <- unlist(str_split(each,""))
  for (a in char){
    score_num <- append(score_num,utf8ToInt(a)-33)
  }
  Score_val <- append(Score_val,list(score_num))
...
>>>>>>> fbcfa0de6b068f6c3c407e5c6ec0b85569f557eb
